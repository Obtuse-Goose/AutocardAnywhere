...
(function($){
  ...
  var noProtocolUrl = /(^|["'(\s]|&lt;)(www\..+?\..+?)((?:[:?]|\.+)?(?:\s|$)|&gt;|[)"',])/g,
      ...
      linkify = $.fn.linkify = async function ( cfg ) {
          ...
          //this.each(async function () {
              var childNodes = this.get(m).childNodes,
                  i = childNodes.length;
              while ( i-- )
              {
                var n = childNodes[i];
                        ...
                        if ( tmpNode.nodeType == 3 )
                        {
                          ...
                          if ( html.length>1  &&  /\S/.test(html) )
                          {
							...
							if ($.isFunction( plugin )) {
								html = await Promise.resolve(plugin(html));
								//console.log(html);
							}
							else {
								html = html.replace( plugin.re, plugin.tmpl );
							}
                ...
                else if ( n.nodeType == 1  &&  !/^(a|button|textarea|script|style)$/i.test(n.tagName) )
                {
